...
export const ServiceDetails = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <Link to="/services" className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-indigo-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="sc-card p-6 space-y-4">
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              <div>
                <span className="text-xs text-slate-400 font-medium block">Fixed Service Fee</span>
                <span className="text-3xl font-extrabold text-slate-900">${service.price}</span>
              </div>

              {role === 'Provider' ? (
                <span className="text-xs text-slate-400 font-medium italic">Viewing in Provider Mode</span>
              ) : (
                <Button onClick={() => setIsBookingOpen(true)} className="py-2.5 px-6">
          ...
          {provider && (
            <div className="sc-card p-6 space-y-4">
              <h3 className="text-base font-bold text-slate-900">About Service Provider</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h4 className="text-base font-bold text-slate-900">{provider.businessName}</h4>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Owner: {provider.fullName}</p>
                  ...
                  <div className="flex items-center gap-2 pt-2 mt-1 border-t border-slate-100">
                    <span className="text-xs text-slate-500 font-bold">🛡 Provider Trust Score:</span>
                    <span className={`px-2 py-0.5 rounded border text-[10px] font-extrabold ${
                      (provider.trustScore !== undefined ? provider.trustScore : 100) >= 90 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                      (provider.trustScore !== undefined ? provider.trustScore : 100) >= 75 ? 'bg-blue-50 text-blue-700 border-blue-100' :
                      (provider.trustScore !== undefined ? provider.trustScore : 100) >= 60 ? 'bg-amber-50 text-amber-700 border-amber-100' :
          ...
          {provider?.reviews && (
            <div className="sc-card p-6 space-y-4">
              <h3 className="text-base font-bold text-slate-900">Customer Reviews ({provider.reviews.length})</h3>

              {provider.reviews.length === 0 ? (
                <p className="text-xs text-slate-400 italic">No reviews submitted yet for this provider.</p>
              ) : (
                <div className="space-y-3">
                  {provider.reviews.map((rev) => (
                    <div key={rev.reviewId} className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1.5">
        ...
        <div className="space-y-6">
          <div className="sc-card p-6 space-y-4">
            <h3 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-3">Service Guarantee</h3>
